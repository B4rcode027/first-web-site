...
const server = http.createServer((req, res) => {
    if (req.method === 'POST' && req.url === '/login') {
        ...
        req.on('end', () => {
            ...
            if (jsonData.username === 'admin' && jsonData.senha === 'senha') {
                ...
                res.end(JSON.stringify({ sucesso: false, message: 'Credenciais inválidas' }));
            }
        });
        return;
    }
    if (req.method === 'POST' && req.url === '/cadastrar') {
        ...
        req.on('end', () => {
            ...
            if (jsonData[chave] === "") {
                camposVazios.push(chave); // Guarda o nome do campo que está vazio
            }
            }
            ...
            // } 
                ...
                if (jsonData.senha !== jsonData.csenha) {
                    res.writeHead(401, { 'Content-Type': 'application/json'});
                    res.end(JSON.stringify({ sucesso: false , message: 'As senhas não coincidem' }));
                    return;
                } 
                    res.writeHead(200,{ 'Content-Type': 'application/json'});
                    res.end(JSON.stringify({ sucesso: true, message: 'Cadastro realizado com sucesso' }));
        })
        return;
